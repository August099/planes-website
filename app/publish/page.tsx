...
export default function PublishPage() {
  // OBTENER SESIÓN DE USUARIO REAL
  ...
  useEffect(() => {
    async function checkUserCredits() {
      ...
      try {
        ...
        if (!res.ok) {
          // Lee el mensaje en texto si la respuesta no es status 200 OK
          const errorText = await res.text();
          console.error(`Error ${res.status} al consultar créditos:`, errorText);
          return;
        }

        const data = await res.json();

        const aircraftBalance = data.aircraftListingsBalance ?? 0;
        const partsBalance = data.sparePartsListingsBalance ?? 0;

        if (activeTab === "aircraft") {
          setIsVerified(aircraftBalance > 0);
        } else {
          setIsVerified(partsBalance > 0);
        }
      } catch (err) {
  ...
  // ==========================================
  // CARGAR OPCIONES DESDE EL API ROUTE
  // ==========================================
  useEffect(() => {
    async function fetchOptions() {
      try {
        const res = await fetch("/api/aircraft-options");
        if (res.ok) {
  ...
  // ==========================================
  // VERIFICACIÓN DE CRÉDITOS USANDO LA SESIÓN REAL
  // ==========================================
  useEffect(() => {
    ...
    const aircraftBalance = user?.aircraftListingsBalance ?? 0;
    const partsBalance = user?.sparePartsListingsBalance ?? 0;

    if (activeTab === "aircraft") {
      if (aircraftBalance <= 0) {
        setIsVerified(false);
        setVerificationMessage("No posees créditos disponibles para publicar una aeronave.");
      } else {
        setIsVerified(true);
      }
      ...
      if (partsBalance <= 0) {
        setIsVerified(false);
        setVerificationMessage("No posees créditos disponibles para publicar repuestos.");
      } else {
        setIsVerified(true);
      }
    }
