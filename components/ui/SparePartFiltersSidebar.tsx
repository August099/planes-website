...
import { SparePartCategory, SparePartCondition } from "@prisma/client";
...
export function SparePartFiltersSidebar() {
  ...
  const sparePartCategories = Object.values(SparePartCategory).map(cat => cat.toLowerCase().replaceAll("_", " "))
  const sparePartConditions = Object.values(SparePartCondition).map(con => con.toLowerCase().replaceAll("_", " "))
  ...
  return (
    <aside className="w-full lg:w-72 shrink-0 border rounded-xl p-5 h-fit sticky top-4 flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1.5">
          {sparePartConditions.map((con) => (
            <label key={con} className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                checked={brands.includes(con)}
                onChange={() => toggleValue(brands, setBrands, con)}
                className="rounded"
              />
              {con}
            </label>
